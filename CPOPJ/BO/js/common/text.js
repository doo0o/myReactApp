...
function updateChar_kko(obj, length_limit)
{
   ...
   if (length > length_limit) {
       alert(argMessage(x2coMessage.getMessage("adminCommon.text.alert.max.byte"), length_limit)); //byte가 최대 이므로 초과된 글자수는 자동으로 삭제됩니다.
...
function updateChar_kko2(obj, length_limit)
{
   ...
   if (length > length_limit) {
       alert(argMessage(x2coMessage.getMessage("adminCommon.text.alert.max.letter"), length_limit)); //글자가 최대 이므로 초과된 글자수는 자동으로 삭제됩니다.
       obj.value = obj.value.replace(/\r\n$/, "");
...
/**
 * timestamp to string (지정된 format 형태로 반환)
 * @param timeValue     : timestamp int형 날짜값
 * @param formatValue   : format string (ex:yyyy-MM-dd HH:mm)
 * @returns
 */
function timestampToFormatString(timeValue, formatValue){
    var retVal = "";
    if(timeValue == "" && formatValue == "") return;

    var date = new Date(timeValue);
    if(formatValue != ""){
        retVal = date.format(formatValue);
    }
    return retVal;
}
...
/**
 * message에 파라미터 반영
 * @param {0}포함한 message문구
 * @param 인자값
 * @returns 인자포함한 문구
 */
function argMessage(str, arg0) {
	if(typeof str != "string" || str.indexOf("{0}") < 0)
		return str;
