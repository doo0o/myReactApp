...
common.Ajax = {
	...
	sendRequest : function(_method, _url, _data, _callback, _async, noLoadingBar, time, loadingBarTxt){
		var that = this;
	    common.showLoadingBar(noLoadingBar, time, loadingBarTxt);
	...
	sendMultipartRequest : function(_method, _url, _data, _callback, noLoadingBar, time, loadingBarTxt){
		var that = this;
		common.showLoadingBar(noLoadingBar, time, loadingBarTxt);
	...
	sendJSONRequest : function(_method, _url, _data, _callback, _async, noLoadingBar, time, loadingBarTxt){
	    var that = this;
	    _async = _async && true;
	    common.showLoadingBar(noLoadingBar, time, loadingBarTxt);
	    ...
	    setTimeout(function() {
	    	$.ajax({
	    		...
	    		,dataType : "json"
    			,success: function(response){
    				that.proceedJson(response, _callback);
    			}
	    		,error	: function (jqXHR,error, errorThrown){
	    			that.error(jqXHR,error, errorThrown);
	    		}
...
//		_callback(that.decodeXss(JSON.stringify(response)));
	...
	proceedJson : function(response, _callback, isJSONP){
		var that = this;
		var jsonObject;
		try{
			if(isJSONP || typeof response === 'object'){
