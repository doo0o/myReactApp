...
//		}
	...
	HeadSticky(){// 페이지 header 상단 고정
		...
		if($('.jQ_sticky').length != 0){// 전시템플릿 상단 흰색 아이콘 
			...
			if($('.model_chart').length != 0){
				const chartT = $('.model_chart').offset().top;
			}
...
function CPOCartCheck(){
	...
	if (chkNameChk == 0) {
		...
		console.log('5개 이상 비교하기 disabled');
	}
...
function CPOCompareSticky(){
	...
	//console.log(heightArr, liHeight )
...
// model = 연식별 텍스트, price = 가격별 텍스트, modelCount = 연식별 바 카운트, priceCount = 가격별 바 카운트, modelPercent = 연식별 바 퍼센트, pricePercent = 가격별 바 퍼센트
function CPOBarChart(model, price, modelCount, priceCount, modelPercent, pricePercent){
	const chartList = $('.list_chart li').length;
	$('.model_chart .sorting li').eq(0).addClass('on');
	$('.model_chart .sorting li').click(function(){
		$('.model_chart .sorting li').removeClass('on');
		$(this).addClass('on');
		for (let i = 0; i <= chartList; i++) {
			if($('.model_chart .sorting li.on').hasClass('model')){
				$('.item0' + i).find('.text').text(model[i - 1]);
				$('.item0' + i).find('em').text(modelCount[i - 1]);
				$('.item0' + i).find('i').css(
					'height',modelPercent[i - 1]
				);
			} else {
				$('.item0' + i).find('.text').text(price[i - 1]);
				$('.item0' + i).find('em').text(priceCount[i - 1]);
				$('.item0' + i).find('i').css(
					'height',pricePercent[i - 1]
				);
			}
		}
	});
